...
class SamPredictor:
    ...
    def set_image(
        ...
        # torch.cuda.reset_max_memory_allocated(self.device)  # 重置最大显存记录
        # torch.cuda.synchronize()  # 确保测量前所有计算完成
        input_image = self.transform.apply_image(image)
        # torch.cuda.synchronize()  # 确保计算完成
        # peak_memory = torch.cuda.max_memory_allocated(self.device)  # 记录峰值显存
        # print(f"peak_memory : {peak_memory/(1024**2):.2f}")
    ...
    def predict_torch(
        ...
        # torch.cuda.reset_peak_memory_stats(boxes.device.type)
        # Embed prompts
        ...
        # peak_memory = torch.cuda.max_memory_allocated(boxes.device.type) / (1024 ** 3)  # 转换为 GB
