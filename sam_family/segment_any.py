...
class SegAny:
    def __init__(self, checkpoint:str, use_bfloat16:bool=True):
        ...
        elif 'Hyper-Sam-HQ-H' in checkpoint:
            from ISAT.segment_any.segment_anything_hq import sam_model_registry, SamPredictor
            self.model_type = "vit_h"
            self.model_source = 'sam_hq'
            print('hyper_sam_hq_h')

        elif 'Hyper-Sam-HQ-L' in checkpoint:
            from ISAT.segment_any.segment_anything_hq import sam_model_registry, SamPredictor
            self.model_type = "vit_l"
            self.model_source = 'sam_hq'
            print('hyper_sam_hq_l')

        elif 'Hyper-Sam-HQ-B' in checkpoint:
            from ISAT.segment_any.segment_anything_hq import sam_model_registry, SamPredictor
            self.model_type = "vit_b"
            self.model_source = 'sam_hq'
            print('hyper_sam_hq_b')


        elif 'Hyper-Sam-H' in checkpoint:
            from ISAT.segment_any.segment_anything import sam_model_registry, SamPredictor
            self.model_type = "vit_h"
            self.model_source = 'sam'
            print('hyper_sam_h')

        elif 'Hyper-Sam-B' in checkpoint:
            from ISAT.segment_any.segment_anything import sam_model_registry, SamPredictor
            self.model_type = "vit_b"
            self.model_source = 'sam'
            print('hyper_sam_b')

        elif 'Hyper-Sam-L' in checkpoint:
            from ISAT.segment_any.segment_anything import sam_model_registry, SamPredictor
            self.model_type = "vit_l"
            self.model_source = 'sam'
            print('hyper_sam_l')
        ...
        ###### Load the model ######
        ###### edit ######
        if 'Hyper-Sam-HQ-H' in checkpoint:
            from .accelerate import init_empty_weights
            from .accelerate import load_checkpoint_and_dispatch

            hyper_inference_mode = 'A'
            model_name = 'SAM-HQ-H'
            rect_l = 0.1
            ...
            SAM_hyper_dic = Decode_Param_Path + '/Compressed_Dir'
            checkpoint_set = SAM_hyper_dic
            # max_memory_set = {0: '10GiB', "cpu": "60GiB"}
            max_memory_set = None
            ...
            sam = load_checkpoint_and_dispatch(
                model,
                checkpoint=checkpoint_set,  # Save_BackParam_Path
                device_map="auto",
                max_memory=max_memory_set,  # 限制 gpu:0 的使用大小，剩余的参数全部加载进cpu
                offload_folder='offload',
                offload_state_dict=True,
                no_split_module_classes=['Block'],
                offload_buffers=True,
                hyper_compress=hyper_compress_set,  # 是否使用hyper_compress文件夹
                hyper_inference_mode=hyper_inference_mode,  # 分层推理模式
                model_name=model_name
            )
