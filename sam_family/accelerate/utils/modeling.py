...
# from lib.Compress_Params_Standard_uint_i_ClassCenterHyper_Multi_VersionC_matrix_CP_best import find_nth_occurrence, decompress_params, decompress2index
from .Compress_Params_Standard_uint_i_ClassCenterHyper_Multi_VersionC_matrix_CP_best import find_nth_occurrence, decompress_params, decompress2index
import torch
...
def set_module_tensor_to_device(
    ...
    """
    # Recurse if needed
    if "." in tensor_name: # 若tensor_name中含有 点"."
    ...
    is_buffer = tensor_name in module._buffers # 该tensor_name是否为buffer
    old_value = getattr(module, tensor_name) # 取出module中对应的tensor
    ...
    if device != "cpu":
        clear_device_cache() # 把 device上的数据清空，只保留cpu上的
...
def load_state_dict_hyper(decode_params_list, LayerName2Param_path):

    with open(LayerName2Param_path, 'rb') as f:
        loaded_checkpoint_dict = pickle.load(f)

    for layer_name in loaded_checkpoint_dict.keys():
        i = loaded_checkpoint_dict[layer_name]
        loaded_checkpoint_dict[layer_name] = decode_params_list[i]

    return loaded_checkpoint_dict




def load_checkpoint_in_model_hyperload(
    model: nn.Module,
...
def load_checkpoint_in_model(
    ...
    Args:
        ...
        strict (`bool`, *optional*, defaults to `False`):
            Whether to strictly enforce that the keys in the checkpoint state_dict match the keys of the model's
            state_dict.
        hyper_compress ('bool', *optional*, defaults to 'False'):
            Whether to load parameters from a hyper_compressed dictory.
    """
    if offload_8bit_bnb:
        from .bnb import quantize_and_offload_8bit
    ...
    if os.path.isfile(checkpoint): # if checkpoint is a path of a file
        if str(checkpoint).endswith(".json"): # if it is a '.json' file
            index_filename = checkpoint
        else:
            checkpoint_files = [checkpoint]  # if it is a '.bin' or '.pth' file

    elif os.path.isdir(checkpoint): # if checkpoint is a path of a dir
        ##### edit ###########
        if hyper_compress:
    ...
    else:  # there is a wrong during loading the checkpoint
        raise ValueError(
    ...
    if index_filename is not None:  # if checkpoint is '.json'
    ...
    for checkpoint_file in checkpoint_files:
        if not hyper_compress:
            ...
            Decode_Param_Path = checkpoint_file[:-15]
            ...
            rect_l_str = Decode_Param_Path[Decode_Param_Path.find(f'{model_name}_l_')+len(f'{model_name}_l_'):] # +6
            rect_l = float(rect_l_str[:1] + '.' + rect_l_str[1:])
            # decode_params_list, time_name, time_list, t0, t1 = decompress_params(
            #     Decode_Param_Path + '/Compressed_Dir/',
            #     rect_l, num_inner_list
            # )

            LayerName2Index, memory_gpu = decompress2index(
                Decode_Param_Path + '/Compressed_Dir/',
                rect_l, num_inner_list
            )
    ...
    return LayerName2Index, rect_l, num_inner_list
