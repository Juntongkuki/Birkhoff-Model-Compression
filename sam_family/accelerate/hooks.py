...
# from .utils.Compress_Params_Standard_uint_i_ClassCenterHyper_Multi_VersionC_matrix_CP_best import decompress_hook
from .utils.Compress_Params_Standard_uint_i_ClassCenterHyper_Multi_VersionC_matrix_CP_CPU import decompress_hook
...
def add_hook_to_module(module: nn.Module, hook: ModelHook, append: bool = False, model_name: Optional[str] = ""):
    """
    ...
    module = hook.init_hook(module, model_name)
...
class AlignDevicesHook(ModelHook):
    ...
    def __init__(
        ...
        hyper_compress: Optional[str] = None,
        ...
        self.hyper_compress = hyper_compress
        self.hyper_inference_mode = hyper_inference_mode
        self.rect_l = rect_l,
        self.num_inner_list = num_inner_list
    ...
    def init_hook(self, module, model_name: Optional[str] = ""):
        ...
        ##### edit ######
        if not self.offload and self.execution_device is not None and "sam" in model_name:
            for name, _ in named_module_tensors(module, recurse=self.place_submodules):
                set_module_tensor_to_device(module, name, "cpu", tied_params_map=self.tied_params_map)
        elif not self.offload and self.execution_device is not None:
            for name, _ in named_module_tensors(module, recurse=self.place_submodules):
            ...
            if module._get_name() == "MaskDecoderHQ":
                self.place_submodules = True
            ...
            for name, _ in named_module_tensors(
                ...
                # if module._get_name() == 'Embedding':
                #     if self.offload:
                #         set_module_tensor_to_device(module, name, 0)
                #     else:
    ...
    def pre_forward(self, module, *args, **kwargs):
        ...
        if module._get_name() == "PromptEncoder":
            self.place_submodules = True
        if module._get_name() == "MaskDecoder":
            self.place_submodules = True
        ...
        if self.offload:
            ...
            for name, _ in named_module_tensors(
                ...
                if self.hyper_inference_mode == 'B':
                    index_array = self.weights_map[name]
                    params = decompress_hook(index_array, self.rect_l[0], self.num_inner_list)
                    value = torch.tensor(params)

                elif self.hyper_inference_mode == 'C':
                    index_array = self.weights_map[name]
                    if index_array['if_decode']:
                        params = decompress_hook(index_array, self.rect_l[0], self.num_inner_list)
                    else:
                        params = index_array['index_array'].reshape(index_array['layer_shape_i'])
                    value = torch.tensor(params)

                elif self.hyper_inference_mode == 'A':
                    index_array = self.weights_map[name]
                    if index_array['if_decode']:
                        params = decompress_hook(index_array, self.rect_l[0], self.num_inner_list, hyper_inference_mode='A')
                    else:
                        params = index_array['index_array'].reshape(index_array['layer_shape_i'])
                    value = torch.tensor(params)
...
def attach_align_device_hook(
    ...
    rect_l: Optional = None,
    num_inner_list: Optional = None,
    hyper_compress: Optional = None,
    hyper_inference_mode: Optional = None
):
    """
    ...
    full_offload = (                                       # module.__class__.__name__ : 这个module的类的名字，例如: "Linear"
        offload and preload_module_classes is not None and module.__class__.__name__ in preload_module_classes
    )

    if len(list(directs)) > 0 or full_offload or module_name == 'prompt_encoder' or module_name == 'image_encoder' or module_name == 'mask_decoder':
...
def attach_align_device_hook_on_blocks(
    ...
    if hyper_save_path == None:
        pass

    elif hyper_save_path != None and 'sam' in model_name:
        if module_name in execution_device and module_name in offload and not offload[module_name]:
            ...
            add_hook_to_module(module, hook, model_name = model_name)
